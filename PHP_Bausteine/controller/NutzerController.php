...
class NutzerController extends BaseController {
    ...
    public function login()
    {
        ...
        $email = $_POST["name"];
        $nutzer = NutzerListe::getInstance() -> getNutzerByEmail($email);

        if(password_verify($password, $nutzer -> getPassword())){
            $_SESSION["id"] = $nutzer -> getNutzerID();
            $_SESSION["email"] = $nutzer -> getEmail();
            $_SESSION["isLoggedIn"] = true;
            $this->redirect("index.php");
        } else {
            $_SESSION["message"] = "login_failure";
            $this->redirect("Einloggen.php");
        }
